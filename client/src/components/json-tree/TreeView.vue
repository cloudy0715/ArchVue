...
const last = arr => (arr = Array.from(arr))[arr.length-1]
const drop = (arr, n = 1) => arr.slice(n)
const dropRight = (arr, n = 1) => arr.slice(0, -n || arr.length)
const cloneDeep = el => JSON.parse(JSON.stringify(el))
  ...
  name: 'tree-view',
    ...
    generateChildrenFromCollection(collection) {
      return Object.entries(collection)
        .map(([keyOrIndex, value]) => {
        if (this.isObject(value)) return this.transformObject(value, keyOrIndex)
        if (this.isArray(value)) return this.transformArray(value, keyOrIndex)
        if (this.isValue(value)) return this.transformValue(value, keyOrIndex)
      })
    },
    ...
    isObject(value) {
      return Boolean(value instanceof Object && !(value instanceof Array))
    },
    isArray(value) {
      return Boolean(value instanceof Array)
    },
    isValue(value) {
